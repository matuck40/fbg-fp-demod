...
voltage the MATLAB overlaid on both right-hand panels comes from the
potentiostat, not the interrogator; given that export, it is drawn the
same way, and without it the axis is simply absent.
...
``fbgfp.io``, optionally with the instrument's Peaks export and the
potentiostat's — the files are read on this machine and nothing leaves it.
File mode opens on the recording shipped in ``sample/``.
...
if source == "Synthetic scenario":
    ...
    cell_path = st.sidebar.text_input(
        ...
        help="A potentiostat (BioLogic) text export, short CCCV or long "
             "form. Its voltage is drawn over both panels on the right, as "
             "the original MATLAB did.",
    )
