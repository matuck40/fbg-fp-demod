...
def linearize(spectrum_db):
    ...
    low = linear.min()
    return (linear - low) / (linear.max() - low)
...
def nearest_valley_pair(valley_wavelengths_nm, reference_nm):
    ...
    demodulated reading. Raises ``ValueError`` when no right neighbour
    exists (the original MATLAB indexed past the end of the array here).
    """
